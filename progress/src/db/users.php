...
function updateUser($user){
    ...
    try {
        ...
        $sql = "UPDATE `users` SET `users_id`=?,`users_name`=?,`users_level`=? WHERE `users_seq`=?";

        $stmt = $pdo->prepare($sql);
        $stmt->execute(array($user->users_id, $user->users_name, intval($user->users_level), $user->users_seq));
