...
$ini = parse_ini_file('../common.ini', FALSE);
    ...
    if (!empty($_SESSION["LEVEL"])) {
        ...
        try {
            ...
            $stmt = $pdo->prepare('SELECT emp.employee_id,emp.name, sch.* FROM `schedule` as sch inner join `employee` as emp on sch.`user_seq` = emp.`employee_seq`  WHERE sch.work_y=? and sch.work_m=? and sch.work_d=? ');
            $stmt->execute(array($ty,$tm,$td));
